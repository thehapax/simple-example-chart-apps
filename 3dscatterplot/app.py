...
app.layout = html.Div([
    html.Div([html.H1("United States Exports")],style={'textAlign': "center","padding-bottom": "10","padding-top": "10"}),
    html.Div([html.Div(dcc.Dropdown(id="select-xaxis",options=[{'label': i.title(), 'value': i} for i in df.columns[3:]],
                                    value='beef',),className="four columns", style={"display":"block","margin-left":"auto",
                                                                                    "margin-right": "auto","width":"33%"}),
        html.Div(dcc.Dropdown(id="select-yaxis",options=[{'label': i.title(), 'value': i} for i in df.columns[3:]],
                              value='pork',),className="four columns", style={"display": "block","margin-left": "auto",
                                                                              "margin-right": "auto","width": "33%"}),
        html.Div(dcc.Dropdown(id="select-zaxis",options=[{'label': i.title(), 'value': i} for i in df.columns[3:]],
                              value='poultry',),className="four columns", style={"display": "block","margin-left":"auto",
                                                                                 "margin-right": "auto","width": "33%" })
...
def ugdate_figure(selected_x, selected_y, selected_z):
    ...
    return {"data": trace,
            "layout": go.Layout(
                ...
                paper_bgcolor="#f3f3f3",scene={"aspectmode": "cube","xaxis": {"title": f"{selected_x.title()} (USD)",},
